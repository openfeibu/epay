...
$sql_phone = "SELECT `com_phone` FROM `pay_user` WHERE `id`=:id";
$stmt = $DB->prepare($sql_phone);
$stmt->execute([":id"=>$_REQUEST["user_name"]]);
$row = $stmt->fetch();
if(!isset($row["com_phone"])){
    echo '{"success":"false","msg":"手机号码未绑定，不能发送短信！"}';
    exit();
}
$json_phone = unserialize($row["com_phone"]);
if(!isset($json_phone["phone"])){
    echo '{"success":"false","msg":"手机号码未绑定，不能发送短信！"}';
    exit();
}

if(isset($_COOKIE[md5("agent_sms_code_login")])){
    date_default_timezone_set("PRC");
    $ser = unserialize($_COOKIE[md5("agent_sms_code_login")]);
    $num = $ser["num"];
    $datetime = null;
    $time = null;
//    var_dump($ser);
...
if($ali_back["Code"] == "OK"){
    ...
    if(isset($_COOKIE[md5("agent_sms_code_login")])){
        $ser = unserialize($_COOKIE[md5("agent_sms_code_login")]);
        $num = $ser["num"]+1;
        setcookie(md5("agent_sms_code_login"),serialize(["num"=>$num,"time"=>$sms_code["time"],"ssk"=>$sms_code["token"],"PhoneNumbers"=>$PhoneNumbers]),$expire);
    }
    else{
        setcookie(md5("agent_sms_code_login"),serialize(["num"=>"$num","time"=>$sms_code["time"],"ssk"=>$sms_code["token"],"PhoneNumbers"=>$PhoneNumbers]),$expire);
    }
