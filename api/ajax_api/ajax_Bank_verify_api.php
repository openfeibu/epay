...
//$str = '{"CDB":"国家开发银行"}';
$bank_name = json_decode($str,true);
$cardNo = $_REQUEST["cardNo"];
$get_back = getCardInfo($cardNo);
if($get_back["validated"]){
    if(isset($bank_name[$get_back["bank"]])){
        echo json_encode(["validated"=>"true","msg"=>"{$bank_name[$get_back["bank"]]}({$get_back["bank"]})"],JSON_UNESCAPED_UNICODE);
    }
    else{
        echo json_encode(["validated"=>"true","msg"=>"未找到({$get_back["bank"]})的银行"],JSON_UNESCAPED_UNICODE);
    }

}
else{
    echo json_encode(["validated"=>"false","msg"=>"请输入正确的卡号！"],JSON_UNESCAPED_UNICODE);
}
//var_dump($get_back);