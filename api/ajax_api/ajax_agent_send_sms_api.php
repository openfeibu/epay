...
$datetime = date("Y-m-d H:i:s",strtotime("+30 seconds",strtotime($phone_json["time"])));
$new_time = date("Y-m-d H:i:s");
if($new_time <= $datetime && $new_time >= $phone_json["time"]){
    echo '{"success":"false","msg":"由于您发送过于频繁，30秒后重试！"}';
    exit();
}
...
if($ali_back["Code"] == "OK"){
    $agent_sms_code["token"] = $token;
    date_default_timezone_set("PRC");
    $agent_sms_code["time"] = date("Y-m-d H:i:s");
    //修改短信验证吗
    $phone_json["pass"] = $token;
    $phone_json["trade_no"] = $trade_no;
    $phone_json["time"] = date("Y-m-d H:i:s");
    $sql_phone_in = "UPDATE `pay_user` SET `com_phone`='".serialize($phone_json)."' WHERE `id`='{$_SESSION["user"]}'";
    $DB->query($sql_phone_in);
    ...
    $num = 1;
    if(isset($_COOKIE[md5("agent_sms_code")])){
        $ser = unserialize($_COOKIE[md5("agent_sms_code")]);
        $num = $ser["num"]+1;
        setcookie(md5("agent_sms_code"),serialize(["num"=>$num,"time"=>$agent_sms_code["time"],"ssk"=>$agent_sms_code["token"],"PhoneNumbers"=>$PhoneNumbers]),$expire);
    }
    else{
        setcookie(md5("agent_sms_code"),serialize(["num"=>"$num","time"=>$agent_sms_code["time"],"ssk"=>$agent_sms_code["token"],"PhoneNumbers"=>$PhoneNumbers]),$expire);
    }
    $ali_back["order_number"] = isset($_REQUEST["order_id"])?$_REQUEST["order_id"]:"000000";
    ll($send_sms_munber);
...
('0','管理云登录@{$send_sms_munber}','{$agent_sms_code["time"]}','{$agent_sms_code["time"]}','{$agent_sms_code["time"]}','{$ip}','{$city1}','{$agent_sms_code["token"]}','{$ali_back}')";
    $DB->query($sql);
