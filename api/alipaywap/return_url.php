...
if($result) {//验证成功
	...
	//支付宝交易号
	$trade_no = htmlspecialchars($_REQUEST['trade_no']);
    ...
    //业务处理代码
    $sql = "SELECT * FROM `pay_order` WHERE trade_no='{$out_trade_no}' AND `status` = '0' limit 1";
    ...
    if(!$srow){
        ...
        file_put_contents(__DIR__."/../log/notify3.log_error.php",date("Y-m-d H:i:s : ").real_ip().$str.PHP_EOL,FILE_APPEND);
        echo "fail";
        exit();
    }
...
//付款完成后，支付系统发送该交易状态通知
    ...
    // file_put_contents(__DIR__."/../log/notify3.log2.php",$call_back,FILE_APPEND);
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
}
else {
    //验证失败
