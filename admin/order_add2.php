...
//创建充值订单
$sql = "INSERT INTO `pay_recharge_record`(`id`,`trade_no`,`out_trade_no`,`type`,`pid`,`addtime`,`name`,`money`) VALUES (NULL,'{$trade_no}','{$out_trade_no}','{$type}','{$userid}','{$date}','{$name}','{$money}')";
//var_dump($sql);
$result = $DB->query($sql);
if(!$result){
    echo "创建充值订单失败，请重试。";
...
switch($type){
    case 'alipay原生接口(废弃)':
        require_once __DIR__.'/../includes/alipay2/config.php';
        //发送订单到支付宝网关
        print <<< EOF
...
EOF;
    ...
    case 'wechat2':
        ...
        require_once __DIR__.DIRECTORY_SEPARATOR."../includes/epay/libs/epay2.php";
        ...
        $post['return_url'] = "{$website_urls}includes/epay/return_url.php";
        $post['notify_url'] = "{$website_urls}includes/epay/notify_url.php";

        $post['name'] = $name;
        $post['sitename'] = $website_name;
        $post['money'] = $money;
        $aop->post = $post;
