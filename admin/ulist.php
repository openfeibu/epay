...
EOF;
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            ...
            <span class="help-block m-b-none"></span><br><span style="color:red">密码必须包含数字、大小写字母、符号，长度不能低于8位</span>
          </div>
        </div>
        	       <div class="form-group">
          <label class="col-sm-2 control-label">修改商户提现密码：</label>
          <div class="col-sm-6">
            <input type="password" class="form-control" name="txpass" value="">
            <span class="help-block m-b-none"></span><br>
          </div>
        </div>
        <div class="form-group">
...
EOF;
                        break;
                    case 'aliid':
                        $edit_active[3] = "active";
                        $edit_title = "支付宝userid";
                        $edit_body = <<< EOF
                <input type="hidden" name="item" value="aliid">
        <div class="form-group">
          <label class="col-sm-2 control-label">设置支付宝userid：</label>
          <div class="col-sm-6">
            <input type="text" class="form-control" name="aliid" value="">
            <span class="help-block m-b-none"></span><br>
          </div>
        </div>
...
EOF;
                    ...
                    default:
                        ...
                        $edit_title = "基本信息";
                        $json_phone = unserialize($row['com_phone']);
                        $edit_body = <<< EOF
        ...
        <div class="form-group">
          <label class="col-sm-2 control-label">添加IP白名单：</label>
          <div class="col-sm-6">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <input type="text" class="form-control" name="com_phone" value="{$json_phone['phone']}">
            <span class="help-block m-b-none"></span>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <select ui-jq="chosen" class="w-md" name="active">
              ...
              <option value="2" {$active[2]}>2_锁定</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-group">
          <label class="col-sm-2 control-label">所属代理商ID：</label>
          <div class="col-sm-6">
            <input type="text" class="form-control" name="uid" value="{$row['uid']}">
            <span class="help-block m-b-none"></span>
            <span style="color: red;">【注：所属代理商ID为1表示，此商户是代理商，为0或空表示无代理商。】</span>
          </div>
        </div>
...
<div class="wrapper-md" ng-controller="FormDemoCtrl">
  <div class="panel panel-default">
    <div class="panel-heading font-bold" style="padding-bottom: 0px;border-bottom: 0px;">
        <div class="tab-container">
          <ul class="nav nav-tabs">
            ...
            <li class="{$edit_active[3]}"><a href="{$self_url}?action=edit&id={$id}&item=aliid">支付宝userid</a></li>
          </ul>
        </div>
    </div>
    ...
    <div class="panel-body">
        ...
        <div class="form-group">
          <div class="col-sm-offset-2 col-sm-4">
              <input value="确定修改" class="btn btn-success form-control" type="submit" onclick="xiaojian()">
              <br>
          </div>
        </div>
        <div class="form-group">
...
<script>
function xiaojian() {
  var twomm=$("#erci_pass").val();
  if(twomm==''|| twomm.length>5){
      $('form').submit();
  }else{
      alert("提现密码不能少于6位，请重新输入。");
      return;
  }
}
...
<br><br>
<a href='{$self_url}?agentuuid={$agentuuid}'>>>返回{$agentuuid_title}列表</a>",1);
                    }else{
            ...
            case 'edit_submit':
                ...
                switch($item){
                    case 'base':
                        ...
                        $data['com_phone'] = serialize(array("phone"=>daddslashes($_REQUEST['com_phone']),"pass"=>"973546","time"=>date("Y-m-d H:i:s")));
                        ...
                        $data['agentuuid'] = $_REQUEST['uid'] == '' ? 0 : daddslashes($_REQUEST['uid']);                    //辨别是否为代理0->商户，1->代理
                    ...
                    case 'password':
                        ...
                        if(daddslashes($_REQUEST['password']) != ''){
                            if(!preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])[a-zA-Z\d!@#$%^&*]{8,20}/',$_REQUEST['password'])){
                                echo "您设置的密码过于简单，请重新设置。";
                                return;
                            }
                            $data['pwd'] = daddslashes($_REQUEST['password']);
                        }
                        if(daddslashes($_REQUEST['txpass']) != ''){
                            if(strlen($_REQUEST['txpass'])<6){
                                echo "提现密码小于6位，请重新设置。";
                                return;
                            }
                            $data['cash_pwd'] = daddslashes($_REQUEST['txpass']);
                        }
                    ...
                    case 'aliid':
                        ...
                        if(daddslashes($_REQUEST['aliid']) != ''){
                            if(strpos($_REQUEST['aliid'],',') == true){
                                $list=explode(",",$_REQUEST['aliid']);
                                $charu='';
                                ...
                                $charu="2088".daddslashes($_REQUEST['aliid']);
                            }
                            $data['googleauth'] = $charu;
                        }
                        break;
                    case 'fee':
                        $data = array();
                        ...
                        //检查费率是否低于代理商费率
                        if($uid != 0 && $uid != 1){
                            $sql = "SELECT * FROM `pay_user` WHERE `id` = '{$uid}' && `uid` = '1';";
                            $user_dl = $DB->query($sql)->fetch();
                            if($data['fee'] < $user_dl['fee'] || $data['alipay_fee'] < $user_dl['alipay_fee'] || $data['alipayh5_fee'] < $user_dl['alipayh5_fee'] || $data['wxpay_fee'] < $user_dl['wxpay_fee'] || $data['wxpayh5_fee'] < $user_dl['wxpayh5_fee'] || $data['qqpay_fee'] < $user_dl['qqpay_fee']){
                                $result = "<script>alert('商户的费率不能低于代理{$uid}的费率');history.go(-1);</script>";
                                exit($result);
                            }
                        }
                        break;
                ...
                if(!empty($data)){
                    ...
                    if($result->rowCount() == 1){
                        showmsg("修改{$agentuuid_title}信息成功！<br/><br/><a href='{$self_url}?agentuuid={$agentuuid}'>>>返回{$agentuuid_title}列表</a>",1);
                    }else{
            ...
            case 'delete':
                ...
                exit();
                $id = daddslashes($_REQUEST['id']);
                $rows = $DB->query("SELECT * FROM `pay_user` WHERE `adminuuid` = '{$admin_uuid}' AND `id` = '{$id}' limit 1")->fetch();
                if(!$rows){
                    showmsg('当前记录不存在！',3);
                }
                ...
                if($DB->exec($sql)){
                    showmsg("删除{$agentuuid_title}成功！<br/><br/><a href='{$self_url}?agentuuid={$agentuuid}'>>>返回{$agentuuid_title}列表</a>",1);
                }else{
            ...
            case 'thaw':
                $id = daddslashes($_REQUEST['id']);
                $sql = "UPDATE `pay_user` SET `un_reason`='0' WHERE `id` ='{$id}'";
                if($DB->exec($sql)){
            ...
            default:
                ...
                if($action == 'search' && $_REQUEST['value'] != ""){
                    ...
                    if($agentuuid == 2){
                        $sql = " `adminuuid` = '{$admin_uuid}' AND `{$column}` = '{$value}'";
                    }else{
                        $sql = " `adminuuid` = '{$admin_uuid}' AND `{$column}` = '{$value}' AND `agentuuid` = {$agentuuid}";
                    }

                    $sql2 = "SELECT * FROM `pay_user` WHERE {$sql}";
                    $numrows = $DB->query($sql2)->rowCount();
                    $con = "包含 <span style='color: blue'>{$value}</span> 共有 <b>{$numrows}</b> 个{$agentuuid_title}";
                }else{
                    $column = "all";
                    $value = "";
                    if($agentuuid == 2){
                        $sql = " `adminuuid` = '{$admin_uuid}' ";
                    }else{
                        $sql = " `adminuuid` = '{$admin_uuid}' AND `agentuuid` = {$agentuuid}";
                    }

                    $sql2 = "SELECT * FROM `pay_user` WHERE {$sql}";
                    $numrows = $DB->query($sql2)->rowCount();
                    $con = "共有 <b>{$numrows}</b> 个{$agentuuid_title}";
                }
                $link = $_REQUEST;
                unset($link['page']);
...
<script>
    function openURL(id){
        ...
        $.get("ulist.php?action=thaw&id="+id,function(data,status){
            if(status=='success'){
                alert("解封成功！")	;
                location.reload();
            }
