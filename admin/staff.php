...
if(!$_SESSION['is_admin'] && $conf['admin_user'] != 'root'){
...
EOF;
    ...
    case 'add_submit':
        ...
        if($account == NULL or $username == NULL){
            ...
            $user['twoauth'] = 0;
            $user['admin_pwd'] = random(12);
            $user['admin_user'] = $account;
            $user['pay_name'] = $username;
            $user['settle_money'] = $money;
            $user['url'] = $url;
            ...
            $user['money_rate'] = 0;
            $result = \epay\admin::create($user);
            if($result['code'] == 1){
                showmsg("添加{$agentuuid_title}成功！<br><br>
<div style='font-size: 18px;color: black;'>登录地址：{$website_urls}admin/<br>{$agentuuid_title}ID：{$result['userid']}<br>商户密码：{$user['admin_pwd']}<br></div>
<br><br>
<a href='{$self_url}?agentuuid={$agentuuid}'>>>返回{$agentuuid_title}列表</a>",1);
    ...
    default:
        ...
        if($action == 'search' && $_REQUEST['column'] != "all"){
            ...
            $numrows = $DB2->fetchRowMany("SELECT * FROM `pay_admin` WHERE `{$column}` = :{$column}",$conds);
            var_dump($numrows);
            ...
            $value = "";
            $sql = " 1 = 1 ";
            $sql2 = "SELECT * FROM `pay_admin` WHERE {$sql}";
            $numrows = $DB->query($sql2)->rowCount();
...
<input type="hidden" name="action" value="search">
  <div class="form-group">
    ...
    <select name="column" class="form-control">
        ...
        <option value="admin_user" {$column_selected['admin_user']}>{$agentuuid_title}账号</option>
        <option value="pay_name" {$column_selected['pay_name']}>管理员姓名</option>
        <option value="web_name" {$column_selected['web_name']}>网站名称</option>
	</select>
  </div>
  <div class="form-group">
    <input type="text" class="form-control" name="value" placeholder="搜索内容" value="{$value}">
  </div>
  <button type="submit" class="btn btn-primary">查询</button>&nbsp;<a href="{$self_url}?action=add" class="btn btn-success">添加{$agentuuid_title}</a>
</form>
...
<div class="table-responsive">
  <table class="table table-striped">
    <thead><tr><th>{$agentuuid_title}ID</th><th>账号</th><th>姓名</th><th>网站名称</th><th>余额</th><th>域名/添加时间</th><th>状态</th><th>操作</th></tr></thead>
    <tbody>
EOF;
        ...
        $sql3 = "SELECT * FROM `pay_admin` WHERE {$sql} order by `id` DESC limit $offset,$pagesize";
        $rs = $DB->query($sql3);
        while($res = $rs->fetch()){
            if(true || $res['active'] == 1){
            ...
            //<td>{$balance}</td>
            //
            echo "<tr><td><b>{$res['id']}</b></td><td>{$res['admin_user']}</td><td>{$res['pay_name']}</td><td>{$res['web_name']}</td><td>0</td><td>{$res['url']}</td><td>{$status}</td><td><a href=\"{$self_url}?action=edit&id={$res['id']}\" class=\"btn btn-xs btn-info\">编辑</a>&nbsp;<a href=\"{$self_url}?action=delete&id={$res['id']}\" class=\"btn btn-xs btn-danger\" onclick=\"return confirm('你确实要删除此{$agentuuid_title}吗？');\" style='display: none;'>删除</a>&ensp;<a href='login_as.php?id={$res['uuid']}&type=staff' class='btn btn-xs btn-success' target='_blank'>作为此管理员身份登录</a></td></tr>";
        }
        echo "
    </tbody>
  </table>
