...
$yesterday = date('Y-m-d',$timenow);
	...
	$sqs=$DB->exec("delete from  `pay_settle` where  time between '{$yesterday} 12:00:01' and '{$yesterday} 23:59:59'");

	echo "delete from  `pay_settle` where  time between '{$yesterday} 12:00:01' and '{$yesterday} 23:59:59'";
...
foreach($listall as $userrows){
	...
	$list=$DB->query("SELECT * FROM pay_order WHERE status>0 and type='alipay' and pid={$pid} and (endtime between '{$yesterday} 12:00:01' and '{$yesterday} 23:59:59') ")->fetchAll();
