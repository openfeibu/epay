  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <form action="{$self_url}?action=add_submit" class="form-horizontal" method="post">
        ...
        <div class="form-group">
          <label class="col-sm-2 control-label">每日额度 <div style="display:inline-block;color: red;">*</div>：</label>
          <div class="col-sm-6">
            <input type="text" class="form-control" name="total_amount" value="0">
            <span>【单位：元，0表示不限制每日额度】</span>
          </div>
        </div>
    ...
    <div class="panel-body">
      <form action="{$self_url}?action=add_submit" class="form-horizontal" method="post">
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <input type="text" class="form-control" name="appid" value="99@{$_SESSION['userid']}" required>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-6">
            <textarea type="text" rows="2" class="form-control" name="public_key" value="" required></textarea>
          </div>
        </div>
...
EOF;
    ...
    case 'edit':
	    ...
	    $sql = "select `cash_pwd` from pay_user WHERE `id`='{$_SESSION["userid"]}' ";
            $cash_pwd = $DB->query($sql)->fetch();
	    $twoauth = daddslashes($_REQUEST['twoauth']);
            // 验证提现密码
            if($twoauth != $cash_pwd['cash_pwd']){
                exit("<script language='javascript'>alert('提现密码错误，请重试。');history.go(-1);</script>");
            }
        }else{
            print <<< EOF
            <div class="bg-light lter b-b wrapper-md">
...
<div class="wrapper-md" ng-controller="FormDemoCtrl">
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <form action="{$self_url}?action=edit&id={$id}" class="form-horizontal" method="post">
          <div class="form-group" style="display: block;">
          <label class="col-sm-2 control-label">提现密码<span style="color: red">*</span>：</label>
          <div class="col-sm-6">
    ...
    <div class="panel-body">
      <form action="{$self_url}?action=edit_submit&id={$id}" class="form-horizontal" method="post">
        ...
        <div class="form-group">
          <label class="col-sm-2 control-label">网站公钥 <div style="display:inline-block;color: red;">*</div>：</label>
          <div class="col-sm-6">
            <textarea type="text" rows="5" class="form-control" name="public_key" required>{$row['public_key']}</textarea>
          </div>
        </div>
        <div class="form-group">
...
EOF;
    ...
    case 'delete':
        ...
        break;
    default:
        $data1 = $DB->query("SELECT `uuid`,`id` from pay_user where `uid` = '{$_SESSION['userid']}'")->fetchAll();
        $sql_uuid = "";
        foreach ($data1 as $value){
            $sql_uuid .= " OR `uuid` = '{$value["uuid"]}' ";
        }
        if($action == 'search'){
            $column = daddslashes($_REQUEST['column']);
            $value = daddslashes($_REQUEST['value']);
            $sql = "SELECT * FROM `pay_channel` WHERE `uuid` = '{$_SESSION['uuid']}' {$sql_uuid} AND `{$column}` LIKE '%{$value}%' ";
			//修正旧代码统计性能BUG
            //$numrows = $DB->query($sql)->rowCount();
			$re=$DB->query("SELECT count(*) as a FROM `pay_channel`  WHERE `uuid` = '{$_SESSION['uuid']}' {$sql_uuid} AND `{$column}` LIKE '%{$value}%' ")->fetch();
			$numrows=$re[0];
            $con = "包含 <span style='color: blue'>{$value}</span> 的共有 <b>{$numrows}</b> 个商户";
        }else{
            $value = "";
            $sql = "SELECT * FROM `pay_channel` WHERE `uuid` = '{$_SESSION['uuid']}' {$sql_uuid} ";
			//修正旧代码统计性能BUG
            //$numrows = $DB->query($sql)->rowCount();
			$re=$DB->query("SELECT count(*) as a FROM `pay_channel` WHERE `uuid` = '{$_SESSION['uuid']}' {$sql_uuid} ")->fetch();
			$numrows=$re[0];
...
<input type="hidden" name="action" value="search">
  ...
  <button type="submit" class="btn btn-primary">搜索</button>&nbsp;<a href="{$self_url}?action=add" class="btn btn-success">添加通道</a>&nbsp;<a href="./ulist.php" class="btn btn-default">商户管理</a>
</form>
...
<div class="table-responsive">
  <table class="table table-striped">
    <thead><tr><th>通道ID(归属商户)</th><th>类型</th><th>APPID</th><th>总额度</th><th>已用额度</th><th>创建时间/更新时间</th><th>状态</th><th>备注</th><th>APP公钥一键配置</th><th>操作</th></tr></thead>
    <tbody>
EOF;
        ...
        while($res = $rs->fetch()){
			...
			echo "<tr><td><b>{$res['id']}({$channel_user})</b><br>{$coundaccept}</td><td>{$res['type']}</td><td>{$res['appid']}</td><td>{$res['total_amount']}</td><td>{$res['used_amount']}</td><td>{$res['createtime']}<br>{$res['updatetime']}</td><td>{$status}</td><td>{$res['note1']}</td>";
			echo "<td><input id='APP_PUBLIC_KEY$kkk' readonly='readonly' onmouseover='APP_PUBLIC_KEY_SHOW(event,this)' style='overflow: hidden;text-overflow:ellipsis;white-space: nowrap;width: 200px;' value='{$APP_PUBLIC_KEY}' >&nbsp;&nbsp;<a href=\"javascript:copyAPP_PUBLIC_KEY($kkk);\"   class=\"btn btn-xs btn-info\">一键复制</a></td>";
			echo "<td><a href=\"{$self_url}?action=edit&id={$res['id']}\" class=\"btn btn-xs btn-info\">编辑</a>&nbsp;<a href=\"{$self_url}?action=delete&id={$res['id']}\" class=\"btn btn-xs btn-danger\" onclick=\"return confirm('你确实要删除此通道吗？');\" style='display: none;'>删除</a>&ensp;</td></tr>";        }
        echo "
    </tbody>
  </table>
</div>
