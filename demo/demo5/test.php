...
<?php
	...
	//file_put_contents("../../config/cache/22.txt",$appid);
     ...
     $userrow = $DB->query($sql)->fetch();
	    ...
	    $params["reqsn"] = $_GET["orderid"];
	    $params["paytype"] = "W01";
	    $params["randomstr"] = "1450432107647";
	    $params["body"] = $goodsid;
	    $params["remark"] = $userrow["body"];
	    $params["acct"] = "";
	    $params["limit_pay"] = "no_credit";
		$params["idno"] = "";
		$params["truename"] = "";
		$params["asinfo"] = "";
        $params["notify_url"] = "http://168.tihou.com/api/notify6.php";
	    ...
	    $rspArray = json_decode($rsp, true); 
	    	...
	    	echo '<img src="http://mobile.qq.com/qrcode?url=' . $rspArray["payinfo"] . '"><br>金额:<font size="5" color="red">'  .$_GET["money"]. '元</font><br><h3>请使用微信扫一扫支付</h3><br>如果你是手机页面打开，保存二维码到手机然后在微信扫一扫中选择“相册”即可支付<br><span style="color:rgba(255,0,0,1.00)">（每个二维码只能支付一次）</span>';
...
setTimeout("javascript:location.href='https://www.baidu.com/'", 50000); 
</script>
</body></html>
