...
if(isset($_POST['user']) && isset($_POST['pass'])){
    ...
    $userrow2 = $DB->query("SELECT * FROM pay_agent WHERE user='{$user}'")->fetch();
    //if($user==$conf['admin_user'] && $pass==$conf['admin_pwd']) {
    //if($user=="GDFS" && $pass=="123446") {
    if($user == $userrow2['user'] && $pass == $userrow2['password']){
        $session = md5($user.$pass.$password_hash);
        $token = authcode("{$user}\t{$session}",'ENCODE',SYS_KEY);
        setcookie("admin_token",$token,time() + 604800);
        //$_SESSION['user_token_fs']=$token;
        @header('Content-Type: text/html; charset=UTF-8');
        exit("<script language='javascript'>alert('登陆管理中心成功！');window.location.href='./';</script>");
    }elseif($pass != $conf['password']){
        @header('Content-Type: text/html; charset=UTF-8');
